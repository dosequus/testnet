...
class Arena:
    ...
    def self_play_game(self, i):
        ...
        try:
            ...
            root = search.Node(None, game.board.fen(), mcts_agent.explore_factor)
            while not game.over():
                root, best_move = mcts_agent.run(root, num_sim=config.train.num_simulations, think_time=3)
            ...
            print(game.board.outcome(claim_draw=True))
    ...
    def compile_batches(self):
        ...
        for state, mask, policy, value in self.memory:
            states.append(state)
            masks.append(mask)
...
def train(model: TakoNet, optimizer: optim.Optimizer, scheduler: optim.lr_scheduler._LRScheduler, starting_epoch=0):
    ...
    for epoch in range(starting_epoch, config.train.num_epochs):
        ...
        for batch in tqdm.tqdm(arena.compile_batches()):
